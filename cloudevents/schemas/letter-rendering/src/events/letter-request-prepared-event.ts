...
export const $LetterRequestPreparedEvent = $EnvelopeProfile.safeExtend({
  type: z.literal("uk.nhs.notify.letter-rendering.letter-request.PREPARED.v1").meta({
...
Data schema version must match the major version indicated by the type`,
  ...
  source: z.string().regex(/^\/data-plane\/letter-rendering(?:\/.*)?$/).meta({
    title: "Event Source",
    description: "Logical event producer path within the letter-rendering domain"
  }),

  subject: z.string().regex(/^client\/[a-z0-9-]+\/letter-request\/[^/]+(?:\/.*)?/).meta({
    title: "Event Subject",
    description: "Resource path (no leading slash) within the source made of segments separated by '/'.",
    examples: ["client/00f3b388-bbe9-41c9-9e76-052d37ee8988/letter-request/0o5Fs0EELR0fUjHjbCnEtdUwQe4_0o5Fs0EELR0fUjHjbCnEtdUwQe5"]
  }),
